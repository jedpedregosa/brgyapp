<?php 
    ...
    if($_FILES["idCardPic"]['size'] > 0 && $_FILES["profilePic"]['size'] > 0) {
        ...
        if(isImageValid($file_dp) && isImageValid($file_id)) {
            $unique_file_name = "RESIDENT_PROOF-" . hash("sha256", $clearance_id . $current_time);
            $file_dir = '../../../FILE_STORAGE/BRGY_PROOF_OF_RES/';
    ...
    $clearance_stmt = "INSERT INTO tblResProof (id, fName, mName, lName, sffx, civStat, ctznshp, 
    ...
    function goPrev($result = 101) {
        ...
        header("Location: ../../e-services/barangay-proof-res");
        exit();
    }
