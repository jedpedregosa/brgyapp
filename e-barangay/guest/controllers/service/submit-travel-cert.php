<?php 
    ...
    if($_FILES["idCardPic"]['size'] > 0 && $_FILES["profilePic"]['size'] > 0) {
        ...
        if(isImageValid($file_dp) && isImageValid($file_id)) {
            $unique_file_name = "RESIDENT_TRAVEL_FILE-" . hash("sha256", $clearance_id . $current_time);
            $file_dir = '../../../FILE_STORAGE/BRGY_TRAVEL/';
    ...
    $clearance_stmt = "INSERT INTO tblTravelRequest (id, fName, mName, lName, sffx, civStat, ctznshp, bDate, sex, hNum, stName, contact, email, fbName, 
    voter, dFname, dMname, dLname, dSffx, dSex, passenger, dHAdd, dBrgy, dZone, dCity, dPlate, dType, sysTime) 
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
    ...
    function goPrev($result = 101) {
        ...
        header("Location: ../../e-services/barangay-travel-cert");
        exit();
    }
