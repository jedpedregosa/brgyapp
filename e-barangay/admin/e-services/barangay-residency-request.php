<?php
    ...
    $req_sql = "SELECT * FROM tblResProof WHERE rStatus = 0";
    
    $isComplete = false;
    if(isset($_GET["type"])) {
        if($_GET["type"] == 1) {
            $req_sql = "SELECT * FROM tblResProof WHERE rStatus = 1";
            $isComplete = true;
        } 
    }
        ...
        <div class = "content request">
            <table class = "grid">
                <tr>
                    ...
                    <td class = "col-right">
                        <a class = "request-status <?php echo (!$isComplete) ? 'status-selected' : ''?>" href = "barangay-residency-request">New Requests</a>
                        <a class = "request-status <?php echo ($isComplete) ? 'status-selected' : ''?>" href = "barangay-residency-request?type=1">Completed Requests</a>
            ...
            <table class = "grid top">
                <tr>
                    ...
                    ?>
                        ...
                        <a href="barangay-residency-request?type=1"  class = "module-selected">PROOF OF RESIDENCY</a>
                    <?php
                }
                ?>
                        </div>
                ...
                <?php 
                    if(!$isComplete) {
                        ?>
                            <input type = "button" onclick = "location.href='reports/barangay-residency-request'" value = "VIEW">
                            <input type = "button" onclick = "completeClick(20)" value = "COMPLETE">
                        <?php
                    }
                ?> 
                            <input type = "button" onclick = "selectClick()" value = "SELECT">
                            <input type = "button" onclick = "deleteClick(21)" value = "DELETE">
                        </div>
                                ...
                                ?>
                                    <tr>
                                        ...
                                        <td><input type = "button" class = "button-3" 
                                                onclick = "showImgModal('<?php echo $hash_id; ?>', 19)" 
                                                value = "view photo">
                                        </td>
                                        ...
                                        <td><input type = "button" class = "button-3" 
                                                onclick = "showImgModal('<?php echo $hash_id; ?>', 20)" 
                                                value = "view ID">
                                        </td>
